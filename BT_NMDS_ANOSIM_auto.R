...
#Jake
    ...
    data.dir <- "C:/Users/Jacob/Desktop/RivGen/Raw_Output07122015/batch0/"    #should only need to change batch number between scenario runs
    wshed <- read.csv("C:/Users/Jacob/Desktop/RivGen/Analyses_Fa2015/WatershedIDsublevels_07122015.csv", header=T)   #update
  
  #Input/Output File Parameters
  scenario <- "Riverine"        #change according to batch number, see line 36 for guidelines     
  species <- "_BT"
  date <- Sys.Date()
  file.ext <- ".gen"
...
#Generations to monitor
  ...
  batch.seq <- 0:3                     #batch 0-3: riverine; batch 4-7: barriers; batch 8-11: panmictic
...
#Create barrier object for ANOSIM ***** MERYL IS THIS THE RIGHT BARRIER CALL, I SAW LIKE 3 different barrier vars
  ...
  for(j in 1:length(gen.seq)){
    for(k in 1:length(batch.seq)){
      #Set up input parameters
        ...
        batch <- paste("batch", fol.no, batch.no, sep="")
      ...
      #-------------------------------------------------------
        ...
        #gen0.anosim <- anosim(gen0.d, barrier)    ## NOT SURE WHERE 'barrier' COMES FROM, HAVE E-MAIL OUT TO MERYL ##
        #summary(gen0.anosim)
